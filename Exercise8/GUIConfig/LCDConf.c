...
*
* Purpose:
*   Get Pixel format for a given layer
*
* Parameter:
...
*
* Parameter:
*   hltdc - pointer to a LTDC_HandleTypeDef structure that contains
...
*
* Purpose:
*   LTDC peripheral interrupt handler
*
* Note(s):
*   (1) 'startup_stm32f746xx.s' defines OTG_FS_IRQHandler() as a WEAK
*       reference. So we re-defined the handler here to overwrite it.
*
...
*   xSize      - X size
*   ySize:     - Y size
*   OffLine:   - offset after each line
...
* Parameter:
*   LayerIndex - Index of layer to be configured
*   IndexSrc   - Index source
*   IndexDst   - Index destination
*
*/
static void CUSTOM_CopyBuffer(int LayerIndex, int IndexSrc, int IndexDst) {
...
*   x            - X position
*   y            - Y position
*   p            - Pointer to position
*   xSize        - X size
*   ySize        - Y size
*   BytesPerLine - Bytes per linePixel index.
*
*/
static void CUSTOM_DrawBitmap32bpp(int LayerIndex, int x, int y, U8 const * p, int xSize, int ySize, int BytesPerLine)
{

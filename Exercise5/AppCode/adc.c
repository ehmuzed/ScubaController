...
void adc_task(void * p_arg)
{
  ...
  OSQCreate(&g_adc_q, "ADC Queue", 22, &err);
  ...
  {
    // : Wait 125 ms.
    OSTimeDlyHMSM(0, 0, 0, 125, 0, &err);    
    // Trigger ADC conversion.
    pot_trigger_conversion();

    // : Wait for message from ADC ISR.
    
    uint16_t data = *((uint16_t*)OSQPend(&g_adc_q, 0,
    ...
    if(err == OS_ERR_NONE)
    {
      snprintf(buf, 64 , "ADC Val: %04u", data);
      GUIDEMO_API_writeLine(2, buf);
      if(data > 1000)
      {
        OSFlagPost (&g_alarm_flags, ALARM_EXTREME, OS_OPT_POST_FLAG_SET, &err);
        GUIDEMO_API_writeLine(7, "Alarm: Extreme");
      }     
      else if(data > 523)
      {
        OSFlagPost (&g_alarm_flags, ALARM_HIGH, OS_OPT_POST_FLAG_SET, &err);
        GUIDEMO_API_writeLine(7, "Alarm: High");
      }
      else if (data > 500)
      {
        OSFlagPost (&g_alarm_flags, ALARM_MEDIUM, OS_OPT_POST_FLAG_SET, &err);
        GUIDEMO_API_writeLine(7, "Alarm: Medium");
      }
      else if (data > 5)
      {
        OSFlagPost (&g_alarm_flags, ALARM_LOW, OS_OPT_POST_FLAG_SET, &err);
        GUIDEMO_API_writeLine(7, "Alarm: Low");
      }
      else
      {
        OSFlagPost (&g_alarm_flags, ALARM_NONE, OS_OPT_POST_FLAG_SET, &err);
        GUIDEMO_API_writeLine(7, "Alarm: None");
      }
    }
  }
}
...
void ADC_IRQHandler(void)
{
  ...
  OSQPost(&g_adc_q, &sample, (OS_MSG_SIZE)4, OS_OPT_POST_FIFO, &err);
